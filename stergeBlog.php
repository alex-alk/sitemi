        ...
        <?php
            if(isset($_COOKIE["user"])){
                if($_COOKIE["user"]=="alex"){
                    ...
                    while ($result !=null){
                        $id = $result['id'];
                        $resultSub = substr($result['titlul'], 0, 400);
                        echo "<article class=delete style='float:left;background-color:white;margin:10px'><a href=stergeBlogBackend.php?id=$id>$count $resultSub</a></article>";
                        $result = $stmt->fetch();
                        $count ++;
                    }
                    $stmt->closeCursor();
